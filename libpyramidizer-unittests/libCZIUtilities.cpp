...
CMemInputOutputStream::CMemInputOutputStream(size_t initial_size, size_t alignment_for_growth) : ptr_(nullptr), alignment_for_growth_(alignment_for_growth), allocated_size_(initial_size), used_size_(0)
{
    if (initial_size > 0)
    {
        this->ptr_ = static_cast<char*>(malloc(initial_size));  // NOLINT
...
CMemInputOutputStream::CMemInputOutputStream(const void* data, size_t size, size_t alignment_for_growth) : CMemInputOutputStream(size, alignment_for_growth)
{
...
void CMemInputOutputStream::EnsureSize(std::uint64_t newSize)
{
    ...
    {
        const size_t new_size_aligned = ((newSize + this->alignment_for_growth_ - 1) / this->alignment_for_growth_) * this->alignment_for_growth_;
        this->ptr_ = static_cast<char*>(realloc(this->ptr_, new_size_aligned));  // NOLINT
        this->allocated_size_ = new_size_aligned;
    }
}
